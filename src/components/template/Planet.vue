  ...
  <div class="planet" @click="handleClick">
    <div class="planet__icon">
      ...
      <template v-if="type === 'planet'">
        ...
        >
          <g>
            <path :fill="color" d="M426.655,444.491c-85.064,74.278-206.9,83.839-299.319,29.581
            ...
            <path :fill="color" d="M107.7,89.244c99.915-87.35,248.817-74.175,333.815,23.051
              c84.998,97.226,75.388,243.379-24.528,330.729c-99.915,87.35-251.727,82.317-336.725-14.908S7.784,176.594,107.7,89.244z"/>

            <g>
              <path :fill="lighter(color, 0.14)" d="M244.029,141.49c-17.92,37.27-63.032,51.341-100.302,33.421
              ...
              <path :fill="darker(color, 0.6)" fill-opacity="0.2" d="M128.086,97.65c17.92-37.27,62.835-54.046,100.105-36.126
            ...
            <path :fill="lighter(color, 0.3)" d="M217.121,218.367c-1.17-5.733,2.71-11.178,8.442-12.348c5.733-1.17,11.248,2.359,12.418,8.091
              c1.17,5.733-2.456,11.466-8.189,12.635C224.06,227.916,218.291,224.099,217.121,218.367z"/>
            <path :fill="lighter(color, 0.5)" d="M363.151,96.945c-1.17-5.733,2.71-11.178,8.442-12.348s11.248,2.359,12.418,8.091
              c1.17,5.733-2.456,11.466-8.189,12.636C370.089,106.493,364.32,102.677,363.151,96.945z"/>

            <path :fill="lighter(color, 0.3)" d="M282.752,398.389c8.691-7.598,21.813-6.256,29.411,2.435c7.598,8.691,6.926,21.591-1.765,29.189
              c-8.691,7.598-22.059,6.972-29.657-1.719C273.143,419.603,274.061,405.987,282.752,398.389z"/>
            <path :fill="lighter(color, 0.5)" d="M58.327,220.961c-1.17-5.733,2.71-11.178,8.442-12.348
              c5.733-1.17,11.248,2.359,12.418,8.091s-2.456,11.466-8.189,12.636C65.265,230.51,59.496,226.694,58.327,220.961z"/>
            <path :fill="lighter(color, 0.3)" d="M468.947,281.701c-3.725,36.649-37.256,62.098-73.905,58.373
      ...
      <template v-else-if="type === 'star'">
        <div
          ...
          :style="{
            width: size + 'px',
            height: size + 'px',
            background: `radial-gradient(circle, ${lighter(color,0.5)} -40%, ${color} 70%, ${darker(color,0.4)} 100%)`,
            boxShadow: `
              0 0 ${size * 0.2}px ${lighter(color, 0.3)},
              0 0 ${size * 0.4}px ${color}80,
              0 0 ${size * 0.8}px ${darker(color, 0.2)}50
            `
          }"
        ></div>
      </template>
